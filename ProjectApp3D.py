...
# Currently set to scale the image by a factor of 3 in every dimension

# In[ ]:

brainT1_img = SimpleITK.Shrink(brainT1_img,[3,3,3])
brainT1c_img = SimpleITK.Shrink(brainT1c_img,[3,3,3])
brainT2_img = SimpleITK.Shrink(brainT2_img,[3,3,3])
brainFlair_img = SimpleITK.Shrink(brainFlair_img,[3,3,3])
initial_seg = SimpleITK.Shrink(initial_seg,[3,3,3])
...
csf = SimpleITK.GetArrayFromImage(SimpleITK.Shrink(SimpleITK.ReadImage(atlas_path + "CSF_warped.nii.gz"),[3,3,3])).flatten()
gm = SimpleITK.GetArrayFromImage(SimpleITK.Shrink(SimpleITK.ReadImage(atlas_path + "GM_warped.nii.gz"),[3,3,3])).flatten()
wm = SimpleITK.GetArrayFromImage(SimpleITK.Shrink(SimpleITK.ReadImage(atlas_path + "WM_warped.nii.gz"),[3,3,3])).flatten()
...
for j in range(0,11): #for now do 10 iterations of EM
